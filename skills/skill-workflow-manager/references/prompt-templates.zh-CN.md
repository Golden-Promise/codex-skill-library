...
| `<import-path>` | 待预检或导入的本地 skill 目录 |
...
```text
$skill-workflow-manager 先预检 <import-path>，不要导入，只告诉我结构是否完整、是否重名、推荐用什么方式导入。
```
...
```text
$skill-workflow-manager 我现在就在目标项目目录。请把 <import-path> 导入当前项目的受管结构，不用我额外再写 project-root。
```
