...
description: Use this skill when the user wants to add, enable, inspect, repair, audit, document, upgrade, or retire Codex skills. It chooses storage paths from context, manages project exposure, and keeps governance state consistent.
---
...
1. Prefer task verbs: `add`, `enable`, `doctor`, `repair`, `audit`, `document`, `upgrade`, `retire`.
2. Let the tool choose shared versus project-owned storage from context and config.
3. Prefer `doctor` before risky changes.
4. Prefer `audit` for CI or release checks.
5. `document` fills missing sections by default; use `--overwrite-skill-md` only when you want a full rewrite.
