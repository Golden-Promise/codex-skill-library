...
当你想在有分量的编码工作前后加一道简洁检查点时，可直接使用这些模板。
如果还需要持续持有任务状态，请继续交给 `skill-context-keeper`。
...
如果需要持续状态归属，请交给 skill-context-keeper。
```
...
剩余风险，以及是否建议交接。
保持简短、偏执行视角。
不要把它扩展成最终交接包，也不要从 skill-context-keeper 手里接管状态归属。
```
